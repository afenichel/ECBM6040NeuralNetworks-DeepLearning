...
class LeNetConvPoolLayer(object):
    ...
    def __init__(self, rng, input, filter_shape, image_shape, poolsize=(2, 2),
        pool_ignore_border=True, gaussian=False):
        """
        ...
        if gaussian==False:
            ...
            for i in range(3):
                sigma=(i+1)**2
                for j, x in enumerate(numpy.linspace(-12,12,5)):
                    for k, y in enumerate(numpy.linspace(-12,12,5)):
                        g[i,j,k]=numpy.exp(-(x**2+y**2)/(2*float(sigma)**2))
                g[i]=g[i]/numpy.sum(g[i])
            g=g.reshape(3,1,5,5)            
            w=numpy.concatenate((g,g,g), axis=1)
            self.W = theano.shared(
                numpy.asarray(w,
                    dtype=theano.config.floatX
                ),
