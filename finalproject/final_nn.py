...
class myMLP(object):
    ...
    def __init__(self, rng, input, n_in, n_hidden, n_out, n_hiddenLayers, 
        ...
        :type activation: theano.Op or function
        :param activation: Non linearity to be applied
        
        :type outputLayer: string
        :param outputLayer: indicates whether to use a Logistic or SVM (Suppor Vector Machine) output layer
        """
        ...
        # This will translate into a series of HiddenLayers with a ReLU activation function 
        # connected to the output layer, either Logistic or SVM; the activation function can 
        # be replaced by sigmoid or any other nonlinear function.
        self.hiddenLayers = []
        ...
        # The logistic/SVM regression layer gets as input the hidden units
        # of the hidden layer
...
def train_nn(decay_learning_rate, train_model, train_model_perf, validate_model, test_model,
    ...
    :param decay_learning_rate: a function defining the learning rate decay
    ...
    param which_data: defines which dataset to train on
    ...
    while (epoch < n_epochs) and (not done_looping):
        ...
        filename=which_data+'_errors.csv'
        if binary and stochastic:
            filename='bin_stochastic_'+filename
        elif binary and not stochastic:
            filename='bin_deterministic_'+filename

        if outputlayer=='Logistic':
            filename='LRout_'+filename
        elif outputlayer=='svm':
            filename='svmout_'+filename


        if not os.path.exists('./output'):
            os.mkdir('./output')
            
        print('epoch %s errors printing to %s' % (epoch, filename))
        if epoch==1:
