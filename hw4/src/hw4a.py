...
class RNNSLU2(object):
    ...
    def __init__(self, nh, ng, nc, ne, de, cs, normal=True):
        ...
        self.h2 = theano.shared(name='h2', value=numpy.zeros(ng, dtype=theano.config.floatX))
        self.wx2 = theano.shared(name='wx2', value=0.2 * numpy.random.uniform(-1.0, 1.0, (nh, ng)).astype(theano.config.floatX))
        self.wh2 = theano.shared(name='wh2', value=0.2 * numpy.random.uniform(-1.0, 1.0, (ng, ng)).astype(theano.config.floatX))
        self.bh2 = theano.shared(name='bh2',value=numpy.zeros(ng, dtype=theano.config.floatX))
        # Last layer parameters
        self.w = theano.shared(name='w', value=0.2 * numpy.random.uniform(-1.0, 1.0, (ng, nc)).astype(theano.config.floatX))
        self.b = theano.shared(name='b', value=numpy.zeros(nc, dtype=theano.config.floatX))
        # bundle
        self.params = [self.emb, self.h0, self.wx, self.wh, self.bh,   
                        self.h2, self.wx2, self.wh2, self.bh2, self.w, self.b]                           
